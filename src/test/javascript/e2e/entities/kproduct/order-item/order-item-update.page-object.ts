...
export default class OrderItemUpdatePage {
  pageTitle: ElementFinder = element(by.id('kstoreApp.kproductOrderItem.home.createOrEditLabel'));
  ...
  async enterData() {
    ...
    await waitUntilDisplayed(this.saveButton);
    await this.setTotalPriceInput('5');
    expect(await this.getTotalPriceInput()).to.eq('5');
    await waitUntilDisplayed(this.saveButton);
    await this.statusSelectLastOption();
    await this.productSelectLastOption();
